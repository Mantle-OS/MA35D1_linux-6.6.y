...
#include "ma35d0-pcm.h"
#include "ma35d0-i2s.h"

static DEFINE_MUTEX(i2s_mutex);
struct ma35d0_i2s_info *ma35d0_i2s_data;
EXPORT_SYMBOL(ma35d0_i2s_data);

static inline void ma35d0_i2s_write_reg(struct ma35d0_i2s_info *info,
                                        unsigned reg, unsigned val)
{
	__raw_writel(val, info->regs + reg);
}

static inline unsigned ma35d0_i2s_read_reg(struct ma35d0_i2s_info *info,
        unsigned reg)
{
                                ...
                                struct snd_soc_dai *dai)
{
	...
	unsigned long val = ma35d0_i2s_read_reg(info, I2S_CTL0);
	...
	ma35d0_i2s_write_reg(info, I2S_CTL0, val);
...
{
	struct ma35d0_i2s_info *info = dev_get_drvdata(cpu_dai->dev);
	unsigned long val = ma35d0_i2s_read_reg(info, I2S_CTL0);
...
static int ma35d0_i2s_set_sysclk(struct snd_soc_dai *cpu_dai, int clk_id, unsigned int freq, int dir)
{
	...
	ma35d0_i2s_write_reg(info, I2S_CLKDIV, (bclkdiv << 8) | mclkdiv);
...
static int ma35d0_i2s_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
	...
	val = ma35d0_i2s_read_reg(info, I2S_CTL0);
...
static void ma35d0_i2s_disable(struct ma35d0_i2s_info *info, int stream)
{
	unsigned long val = ma35d0_i2s_read_reg(info, I2S_CTL0);
...
static int ma35d0_i2s_startup(struct snd_pcm_substream *substream,
                              struct snd_soc_dai *dai)
{
	struct ma35d0_i2s_info *info = snd_soc_dai_get_drvdata(dai);

	//printk("Enter %s.....\n", __FUNCTION__);

	ma35d0_i2s_enable(info, substream->stream);

	return 0;
}

static void ma35d0_i2s_shutdown(struct snd_pcm_substream *substream,
                                struct snd_soc_dai *dai)
{
	struct ma35d0_i2s_info *info = snd_soc_dai_get_drvdata(dai);
	...
	ma35d0_i2s_disable(info, substream->stream);
}
...
static struct snd_soc_dai_ops ma35d0_i2s_dai_ops = {
	...
	*/
	...
	.startup        = ma35d0_i2s_startup,
	.shutdown       = ma35d0_i2s_shutdown,
	.hw_params      = ma35d0_i2s_hw_params,
	.set_sysclk     = ma35d0_i2s_set_sysclk,
	.set_fmt        = ma35d0_i2s_set_fmt,
	.trigger        = ma35d0_i2s_trigger,
...
static int ma35d0_i2s_drvprobe(struct platform_device *pdev)
{
	...
	info = devm_kzalloc(&pdev->dev, sizeof(struct ma35d0_i2s_info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;

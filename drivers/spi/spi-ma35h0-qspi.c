...
static void ma35h0_slave_txdma_callback(void *arg)
{
	struct nuvoton_spi *hw = (struct nuvoton_spi *)arg;
	struct ma35h0_dma_done *done = &hw->dma_slave_txdone;
...
static void ma35h0_slave_rxdma_callback(void *arg)
{
	struct nuvoton_spi *hw = (struct nuvoton_spi *)arg;
	struct ma35h0_dma_done *done = &hw->dma_slave_rxdone;
...
{
	struct ma35h0_ip_dma *pdma = &hw->dma;
	...
	if (hw->pdata->use_pdma) {
		...
		if (len < USE_PDMA_LEN) {
			...
			if (rxbuf) {
				...
				pdma->rxdesc->callback = ma35h0_slave_rxdma_callback;
			...
			if (txbuf) {
				...
				pdma->txdesc->callback = ma35h0_slave_txdma_callback;
...
MODULE_DESCRIPTION("Nuvoton MA35H0 QSPI Controller driver!");

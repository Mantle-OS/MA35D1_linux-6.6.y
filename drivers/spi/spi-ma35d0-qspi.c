...
struct nuvoton_spi {
	...
	spinlock_t                      lock;
	struct resource                 *res;
	struct ma35d0_dma_done          dma_slave_rxdone;
	struct ma35d0_dma_done          dma_slave_txdone;
	struct ma35d0_ip_dma            dma;
...
static void ma35d0_slave_txdma_callback(void *arg)
{
	struct nuvoton_spi *hw = (struct nuvoton_spi *)arg;
	struct ma35d0_dma_done *done = &hw->dma_slave_txdone;
...
static void ma35d0_slave_rxdma_callback(void *arg)
{
	struct nuvoton_spi *hw = (struct nuvoton_spi *)arg;
	struct ma35d0_dma_done *done = &hw->dma_slave_rxdone;
...
{
	struct ma35d0_ip_dma *pdma = &hw->dma;
	...
	if (hw->pdata->use_pdma) {
		...
		if (len < USE_PDMA_LEN) {
			...
			if (rxbuf) {
				...
				pdma->rxdesc->callback = ma35d0_slave_rxdma_callback;
			...
			if (txbuf) {
				...
				pdma->txdesc->callback = ma35d0_slave_txdma_callback;
...
MODULE_DESCRIPTION("Nuvoton MA35D0 QSPI Controller driver!");

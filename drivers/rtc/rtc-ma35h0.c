...
#define MA35H0_RTC_NVMEM_SIZE	(16 * 4)
...
static int ma35h0_rtc_nvram_read(void *priv, unsigned int offset,
				 void *val, size_t bytes)
{
	struct ma35h0_rtc *rtc = priv;
	u8 *buf = val;
	unsigned int i;

	if (offset + bytes > MA35H0_RTC_NVMEM_SIZE)
		return -EINVAL;
...
static int ma35h0_rtc_nvram_write(void *priv, unsigned int offset,
				  void *val, size_t bytes)
{
	struct ma35h0_rtc *rtc = priv;
	const u8 *buf = val;
...
static int ma35h0_rtc_probe(struct platform_device *pdev)
{
	...
	struct nvmem_config nvmem_cfg = {
		.name = "ma35h0-rtc-nvmem",
		.stride = 1,
		.word_size = 1,
		.size = MA35H0_RTC_NVMEM_SIZE,
		.type = NVMEM_TYPE_BATTERY_BACKED,
		.reg_read = ma35h0_rtc_nvram_read,
		.reg_write = ma35h0_rtc_nvram_write,
	};

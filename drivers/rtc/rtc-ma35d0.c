...
#define MA35D0_RTC_NVMEM_SIZE	(16 * 4)
...
static u32 rtc_reg_read(struct ma35d0_rtc *rtc, int offset)
{
	return readl_relaxed(rtc->base + offset);
}

static void rtc_reg_write(struct ma35d0_rtc *rtc, int offset, int value)
{
...
static int ma35d0_rtc_bcd2bin(u32 timereg, u32 calreg, u32 wdayreg, struct rtc_time *tm)
{
...
static int ma35d0_rtc_alarm_bcd2bin(u32 timereg, u32 calreg, struct rtc_time *tm)
{
...
static int ma35d0_rtc_nvram_read(void *priv, unsigned int offset,
				 void *val, size_t bytes)
{
	struct ma35d0_rtc *rtc = priv;
	u8 *buf = val;
	unsigned int i;

	if (offset + bytes > MA35D0_RTC_NVMEM_SIZE)
		return -EINVAL;
...
static int ma35d0_rtc_nvram_write(void *priv, unsigned int offset,
				  void *val, size_t bytes)
{
	struct ma35d0_rtc *rtc = priv;
	const u8 *buf = val;
...
{
	struct ma35d0_rtc *rtc;
	struct clk *clk;
	u32 val32;
	int err;
	struct nvmem_config nvmem_cfg = {
		.name = "ma35d0-rtc-nvmem",
		.stride = 1,
		.word_size = 1,
		.size = MA35D0_RTC_NVMEM_SIZE,
		.type = NVMEM_TYPE_BATTERY_BACKED,
		.reg_read = ma35d0_rtc_nvram_read,
		.reg_write = ma35d0_rtc_nvram_write,
	};

	rtc = devm_kzalloc(&pdev->dev, sizeof(struct ma35d0_rtc), GFP_KERNEL);
	if (!rtc)
		return -ENOMEM;
	...
	if (devm_request_irq(&pdev->dev, rtc->irq, ma35d0_rtc_interrupt, 0,
			     "ma35d0rtc", rtc)) {
...
{
	struct ma35d0_rtc *rtc = platform_get_drvdata(pdev);
...
static struct platform_driver ma35d0_rtc_driver = {
	...
	.driver		= {
		.name	= "ma35d0-rtc",
		.owner	= THIS_MODULE,
		.of_match_table = ma35d0_rtc_of_match,
	},
};
